...
6. **Read the cross-cutting spec** — the registry is **materialized up-front from Architecture §7/§8**, so treat the contracts naming this component as *resolved, authoritative-by-reference obligations* (not provisional). Realize a contract you **produce** so it conforms to the registered obligation; for a contract you **consume** that carries a `**Binds**: PRD §5 …` pointer, honour the bound field list as the payload shape you can rely on. Divergence from a registered contract is a cross-boundary requirement, not a silent local choice.
