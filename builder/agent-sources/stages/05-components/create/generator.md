...
- Cross-cutting spec
- Cross-cutting deferred items — provisional cross-component conventions this component may adopt (governed upstream — there is no cross-cutting-spec ratifying author). Distinct from `cross-cutting.md` itself, which is **materialized up-front from Architecture §7/§8** as a resolved contract layer.
...
6. **Read the cross-cutting spec** — the registry is **materialized up-front from Architecture §7/§8**, so treat the contracts naming this component as *resolved, authoritative-by-reference obligations* (not provisional). Realize a contract you **produce** so it conforms to the registered obligation; for a contract you **consume** that carries a `**Binds**: PRD §5 …` pointer, honour the bound field list as the payload shape you can rely on. Divergence from a registered contract is a cross-boundary requirement, not a silent local choice. **Also read the step-0 interface schema-specs** (`system-design/05-components/specs/cross-cutting-interfaces/`, if any exist): for a §7 cross-cutting interface this component **writes to or reads** (e.g. an audit trail it writes), adopt the frozen schema-spec **by reference** — its schema, write-signature, write posture, reason-taxonomy, and retention are the shared shape every writer/reader must be consistent on. Realize your component's use of the interface to conform to it; do **not** invent a divergent schema/posture for a shared interface. This is non-blocking (adopt by reference where relevant; a component with no such interface simply has nothing to adopt).
7. **Read the cross-cutting deferred items** — provisional cross-component conventions. Treat these as working conventions this component adopts unless there is specific reason to deviate. Record each adopted convention in §12 Pending Issues as a cross-reference to the cross-cutting DEF-ID (traceability, not a pending action — there is no cross-cutting-spec ratification step; the registry itself is materialized up-front from §7/§8, not authored here). If a convention is contested or cannot be adopted cleanly, raise it as a **cross-boundary requirement** (see `docs/cross-boundary-requirements.md`) rather than deferring to a non-existent cross-cutting authoring pass.
...
### Step 0a: Adopt Cross-Cutting Provisional Conventions
