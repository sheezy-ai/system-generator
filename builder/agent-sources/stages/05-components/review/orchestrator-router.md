...
1b. **Record upstream freshness (per-component, the reconciled-against watermark — TOCTOU-safe).** If this round ran the Alignment Verifier (post-discussion Step 8 — i.e. reached here via **EXIT**, not the **ZERO_ISSUES** path), advance this component's per-edge freshness record to **the `Frozen-At` the AV actually read**:
   - Read this round's `round-[R]-review-[build|ops]/07-alignment-report.md` `## Frozen-At Read (per source)` block; extract the tokens the AV read for **02-prd**, **03-foundations**, and **04-architecture** (the 05 AV's three direct sources — `alignment-verifier.md` source table).
   - Write/update the `## Upstream Freshness (reconciled-against)` block in **this component's** `{{SYSTEM_DESIGN_PATH}}/system-design/05-components/versions/[component]/workflow-state.md`: set `- 02-prd: [token]`, `- 03-foundations: [token]`, `- 04-architecture: [token]`. These are held and gated **per component** — `05-init`'s one-time registry check is a setup precondition, not the mid-stage backstop, so the 04 edge is held here like the others.
   - Record `ABSENT` for any source the AV read as `ABSENT`; the per-component Promote guard treats an absent-source token as inert (a no-op), not stale.
   - **ZERO_ISSUES fast-path caveat:** if this completion was reached via pre-discussion's **ZERO_ISSUES** return (no Author, no post-discussion, no Step-8 AV), it advances **no** edge — leave the record unchanged. The per-component Promote guard then correctly forces a real Review round before the component can freeze.
