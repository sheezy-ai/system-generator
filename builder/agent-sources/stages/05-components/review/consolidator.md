...
4. **Read pending-issues.md** (if provided) — extract UNRESOLVED pending issues for this component (to include, per "Handling Pending Issues") **and** read the `## Resolved Issues` section's `Concern key`s (to suppress re-raises, per "Handling Resolved Issues")
...
`pending-issues.md` also carries a `## Resolved Issues` section recording concerns closed in prior rounds — including **dismissed / no-spec-change** items, each with a `**Concern key:**` (spec-section anchor + concern summary). These are the re-raise risk: because a dismissal leaves the spec text unchanged, an expert re-scanning the spec fresh can surface the identical concern as if it were new, re-triggering another round. This is what makes the review loop fail to converge.

For each incoming expert issue, **before** consolidating it:

1. **Match it against the Resolved `Concern key`s.** A match means the incoming issue targets the **same spec section** and raises **substantially the same concern** as a resolved-and-dismissed entry. (Match on section + concern gist — round-local SPEC-NNN IDs do not persist, so you cannot match on ID.)
2. **If it matches AND the cited spec section is materially unchanged** since the dismissal: keep the issue in the output but **tag it** `[RE-RAISE — dismissed in Round N: <one-line prior rationale>]`, and note its default disposition is **DROP unless the expert supplies a genuinely new fact** (new evidence, a since-changed section, or a consequence not previously considered). Do **NOT** silently delete it — the tag lets the Issue Router / decision-triage / human confirm the prior dismissal still stands rather than re-litigate it from scratch.
3. **If the spec section HAS materially changed** since the dismissal: do **not** tag it — the change may legitimately reopen the concern. Consolidate normally.

This gives the review loop a memory: a concern the human already dismissed does not silently reappear as a fresh HIGH/MEDIUM and spin up another round. (APPLIED-inline resolutions need no suppression — the spec text already reflects the fix, so a re-scan sees the change.)
...
**Output file**: `[OUTPUT_DIR]/round-[N]-review-[build|ops]/02-consolidated-issues.md`
