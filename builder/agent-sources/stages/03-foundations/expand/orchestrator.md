# Foundations Expand Orchestrator

---

## Purpose

Expand the Foundations by adding new capability areas or scope changes to an existing, promoted document. Unlike Create (which generates from scratch) or Review (which critiques and refines), Expand adds new content driven by a specific trigger — a downstream discovery, a scope decision, or a human-identified gap.

The expand workflow produces an updated Foundations that reads as if the expanded capability was always in scope. It does not promote — the next step is always a Review round.
...
**State file**: `system-design/03-foundations/versions/workflow-state.md`
**Working files**: `system-design/03-foundations/versions/round-[N]-expand/`
...
Note: Change verification for expand uses the review workflow's change-verifier agent (`{{AGENTS_PATH}}/03-foundations/review/change-verifier.md`) since the verification logic is the same.
...
```
system-design/03-foundations/versions/
├── workflow-state.md
├── round-[N]-expand/
...
│   ├── 05-updated-foundations.md      # Updated Foundations with expansion applied
...
2. **Determine trigger input**:
   - If a pending issues file exists with unresolved issues: use `system-design/03-foundations/versions/pending-issues.md`
...
3. **Run Scope Analyst**:
   ```
   Follow the instructions in: {{AGENTS_PATH}}/03-foundations/expand/scope-analyst.md

   Trigger: system-design/03-foundations/versions/round-[N]-expand/00-trigger.md
   ...
   Output: system-design/03-foundations/versions/round-[N]-expand/01-expansion-brief.md
   ```
...
10. **Spawn Expansion Explorers** (one per capability area, in parallel):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/03-foundations/expand/expansion-explorer.md

    Expansion Brief: system-design/03-foundations/versions/round-[N]-expand/01-expansion-brief.md
    Capability area: [CAP-ID]
    Foundations: system-design/03-foundations/versions/round-[N]-expand/00-foundations.md
    Blueprint: system-design/01-blueprint/blueprint.md
    Stage guide: {{GUIDES_PATH}}/03-foundations-guide.md
    Output: system-design/03-foundations/versions/round-[N]-expand/02-explorer-{cap-name}.md
    ```
...
17. **Run Proposal Filter** to check level-appropriateness and format for human review:
    ...
    Output: system-design/03-foundations/versions/round-[N]-expand/03-expansion-review.md
    ```
...
21. **Notify user** proposals are ready for review:
    - Point them to `system-design/03-foundations/versions/round-[N]-expand/03-expansion-review.md`
...
23. **Discussion loop**:
    ...
    b. **Invoke Discussion Facilitator agents** (batched by capability area):
       ...
       Issues file: system-design/03-foundations/versions/round-[N]-expand/03-expansion-review.md
       Issues: [ID1, ID2, ...]
       ```
...
27. **Run Integration Author**:
    ```
    Follow the instructions in: {{AGENTS_PATH}}/03-foundations/expand/integration-author.md

    Foundations: system-design/03-foundations/versions/round-[N]-expand/00-foundations.md
    Approved proposals: system-design/03-foundations/versions/round-[N]-expand/03-expansion-review.md
    Stage guide: {{GUIDES_PATH}}/03-foundations-guide.md
    Change log output: system-design/03-foundations/versions/round-[N]-expand/04-integration-output.md
    Updated Foundations output: system-design/03-foundations/versions/round-[N]-expand/05-updated-foundations.md
    ```
...
31. **Spawn all four verification agents in parallel**:
    ...
    **Change Verifier** (Step 6):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/03-foundations/review/change-verifier.md

    Issues summary with resolutions: system-design/03-foundations/versions/round-[N]-expand/03-expansion-review.md
    Author output: system-design/03-foundations/versions/round-[N]-expand/04-integration-output.md
    Updated Foundations: system-design/03-foundations/versions/round-[N]-expand/05-updated-foundations.md
    Output: system-design/03-foundations/versions/round-[N]-expand/06-change-verification.md
    ```
    ...
    **Alignment Verifier** (Step 7):
    ...
    Updated Foundations: system-design/03-foundations/versions/round-[N]-expand/05-updated-foundations.md
    Blueprint: system-design/01-blueprint/blueprint.md
    Output: system-design/03-foundations/versions/round-[N]-expand/07-alignment-report.md
    ```
    ...
    **Internal Coherence Checker** (Step 8):
    ...
    Document: system-design/03-foundations/versions/round-[N]-expand/05-updated-foundations.md
    Stage guide: {{GUIDES_PATH}}/03-foundations-guide.md
    Output: system-design/03-foundations/versions/round-[N]-expand/08-coherence-report.md
    ```
    ...
    **Enumeration Verifier** (Step 9):
    ...
    Document: system-design/03-foundations/versions/round-[N]-expand/05-updated-foundations.md
    Stage guide: {{GUIDES_PATH}}/03-foundations-guide.md
    Output: system-design/03-foundations/versions/round-[N]-expand/09-enumeration-report.md
    ```
...
45. **Handle pending issue sync** (if alignment verification found SYNC_UPSTREAM items):
    ...
    - Apply the same **menu-build suppression** as the review path before finalising sync decisions: cross-check each finding against its target register with the shared matcher (`target-stage + section-anchor + concern-gist`, semantic, staleness-gated); a `WONT_FIX` match is shown inline tagged `[RE-RAISE]` (default-drop), an `UNRESOLVED` match is annotated "already logged, awaiting upstream", a materially-changed upstream section reopens, uncertain ⇒ show
...
45b. **Close consumed escalations in this stage's own pending-issues (cross-stage re-raise ledger)**: for each `AWAITS_UPSTREAM_REVISION` (P2) escalation this round consumed at Step 1, transition its entry in `system-design/03-foundations/versions/pending-issues.md` and move it to `## Resolved Issues`:
    - **Integrated** (an accepted proposal addressed it): `**Status:** RESOLVED` · `**Resolved:** [date]` · `**Resolution Round:** Foundations Expand Round [N]` · `**Resolution:** [Foundations section changed]`.
    - **Declined** (no accepted proposal addresses it — the originating spec must rewrite to match, per `cross-boundary-requirements.md` Non-Ratification Fallback): `**Status:** WONT_FIX` · `**Resolved:** [date]` · `**Resolution Round:** Foundations Expand Round [N]` · `**Resolution:** [why the escalation was declined — the human-approved rationale]` · `**Concern key:** [Foundations section anchor] — [one-line concern summary]`. The Concern key is what the **next** Expand batch's cross-stage suppression (Step 1) matches against, so the same concern — if re-escalated later by another component — does not silently re-trigger a round.
