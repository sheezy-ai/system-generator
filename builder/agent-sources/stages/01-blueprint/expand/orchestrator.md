# Blueprint Expand Orchestrator

---

## Purpose

Expand the Blueprint by adding new capability areas or scope changes to an existing, promoted document. Unlike Create (which generates from scratch) or Review (which critiques and refines), Expand adds new content driven by a specific trigger — a downstream discovery, a scope decision, or a human-identified gap.

The expand workflow produces an updated Blueprint that reads as if the expanded capability was always in scope. It does not promote — the next step is always a Review round.
...
- [ ] Step 8: Enumeration Verification
...
**State file**: `system-design/01-blueprint/versions/workflow-state.md`
**Working files**: `system-design/01-blueprint/versions/round-[N]-expand/`
...
Note: Change verification for expand uses the review workflow's change-verifier agent (`{{AGENTS_PATH}}/01-blueprint/review/change-verifier.md`) since the verification logic is the same.
...
│   ├── 05-updated-blueprint.md        # Updated Blueprint with expansion applied
│   ├── 06-change-verification.md
│   ├── 07-coherence-report.md
│   ├── 08-enumeration-report.md
...
2. **Determine trigger input**:
   - If a pending issues file exists with unresolved issues: use `system-design/01-blueprint/versions/pending-issues.md`
...
3. **Run Scope Analyst**:
   ```
   Follow the instructions in: {{AGENTS_PATH}}/01-blueprint/expand/scope-analyst.md

   Trigger: system-design/01-blueprint/versions/round-[N]-expand/00-trigger.md
   Blueprint: system-design/01-blueprint/versions/round-[N]-expand/00-blueprint.md
   Stage guide: {{GUIDES_PATH}}/01-blueprint-guide.md
   Output: system-design/01-blueprint/versions/round-[N]-expand/01-expansion-brief.md
   ```
...
10. **Spawn Expansion Explorers** (one per capability area, in parallel):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/01-blueprint/expand/expansion-explorer.md

    Expansion Brief: system-design/01-blueprint/versions/round-[N]-expand/01-expansion-brief.md
    Capability area: [CAP-ID]
    Blueprint: system-design/01-blueprint/versions/round-[N]-expand/00-blueprint.md
    Stage guide: {{GUIDES_PATH}}/01-blueprint-guide.md
    Output: system-design/01-blueprint/versions/round-[N]-expand/02-explorer-{cap-name}.md
    ```
...
12. **If multiple capability areas** (2+), run Exploration Consolidator:
    ...
    Blueprint: system-design/01-blueprint/versions/round-[N]-expand/00-blueprint.md
    Output: system-design/01-blueprint/versions/round-[N]-expand/03-consolidated-proposals.md
    ```
...
17. **Run Proposal Filter** to check level-appropriateness and format for human review:
    ```
    Follow the instructions in: {{AGENTS_PATH}}/01-blueprint/expand/proposal-filter.md

    Stage guide: {{GUIDES_PATH}}/01-blueprint-guide.md
    Input: system-design/01-blueprint/versions/round-[N]-expand/03-consolidated-proposals.md
    Output: system-design/01-blueprint/versions/round-[N]-expand/03-expansion-review.md
    ```
...
21. **Notify user** proposals are ready for review:
    - Point them to `system-design/01-blueprint/versions/round-[N]-expand/03-expansion-review.md`
...
23. **Discussion loop**:
    ...
    b. **Invoke Discussion Facilitator agents** (batched by capability area):
       ...
       Issues file: system-design/01-blueprint/versions/round-[N]-expand/03-expansion-review.md
       Issues: [ID1, ID2, ...]
       ```
...
27. **Run Integration Author**:
    ```
    Follow the instructions in: {{AGENTS_PATH}}/01-blueprint/expand/integration-author.md

    Blueprint: system-design/01-blueprint/versions/round-[N]-expand/00-blueprint.md
    Approved proposals: system-design/01-blueprint/versions/round-[N]-expand/03-expansion-review.md
    Stage guide: {{GUIDES_PATH}}/01-blueprint-guide.md
    Change log output: system-design/01-blueprint/versions/round-[N]-expand/04-integration-output.md
    Updated Blueprint output: system-design/01-blueprint/versions/round-[N]-expand/05-updated-blueprint.md
    ```

28. **Update state file**: Mark Step 5 complete

29. **Automatically proceed to Steps 6-8**
...
31. **Spawn all three verification agents in parallel**:
    ...
    **Change Verifier** (Step 6):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/01-blueprint/review/change-verifier.md

    Issues summary with resolutions: system-design/01-blueprint/versions/round-[N]-expand/03-expansion-review.md
    Author output: system-design/01-blueprint/versions/round-[N]-expand/04-integration-output.md
    Updated Blueprint: system-design/01-blueprint/versions/round-[N]-expand/05-updated-blueprint.md
    Output: system-design/01-blueprint/versions/round-[N]-expand/06-change-verification.md
    ```

    **Internal Coherence Checker** (Step 7):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/universal-agents/internal-coherence-checker.md

    Document: system-design/01-blueprint/versions/round-[N]-expand/05-updated-blueprint.md
    Stage guide: {{GUIDES_PATH}}/01-blueprint-guide.md
    Output: system-design/01-blueprint/versions/round-[N]-expand/07-coherence-report.md
    ```

    **Enumeration Verifier** (Step 8):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/universal-agents/enumeration-verifier.md

    Document: system-design/01-blueprint/versions/round-[N]-expand/05-updated-blueprint.md
    Stage guide: {{GUIDES_PATH}}/01-blueprint-guide.md
    Output: system-design/01-blueprint/versions/round-[N]-expand/08-enumeration-report.md
    ```
...
34. **Automatically proceed to Step 9**
...
36. **Read all verification reports** and aggregate findings:
    ...
    - `08-enumeration-report.md` — check for HIGH or MEDIUM missing items
...
38. **Track rework pass count**: Count how many times Steps 5→6-8→9 have executed. Record in state file history.

39. **Determine next action based on Overall Status**:

    a. **If CLEAN**: Mark Step 9 complete, proceed to Step 10
...
40. **If NEEDS_DECISIONS, present to human** (same format as review orchestrator):
    ...
    Present relevant sections (change issues, coherence gaps, enumeration gaps) with FIX/ACCEPT options per item.
...
43. **Update state file**: Mark Step 9 complete, proceed to Step 10

---

### Step 10: Route

44. **Update state file**: Set Step 10, status = IN_PROGRESS

45. **Update state file**: Mark Step 10 complete

46. **Notify human**: Expand round complete.
    ```
...
47. **Update state file**: Set status = COMPLETE
...
- Steps 5 → 6+7+8 (parallel) → 9: Integration through verification without pausing
...
- **Step 9** (if NEEDS_DECISIONS) — Present verification results, collect decisions
