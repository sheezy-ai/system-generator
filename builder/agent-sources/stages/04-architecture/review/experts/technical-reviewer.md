...
- **HIGH**: Feasibility / Foundations-alignment issue whose consequence is implementation-blocking, names a security risk class, or requires rework spanning multiple components or specs (including upstream document changes).
- **MEDIUM**: Technical concern with a named concrete consequence addressable by a single component spec author or operator without rework cascade.
- **LOW**: Technical issue with a real but minor concrete consequence — single sentence or row edit at architecture level, no downstream rework. Do not use LOW as a catch-all for "would improve."
