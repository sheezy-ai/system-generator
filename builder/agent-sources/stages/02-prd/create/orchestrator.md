...
Initialize the PRD stage by setting up structure, generating a draft PRD from the Blueprint, resolving any gaps through structured discussion, and promoting the final draft to `prd.md` for the Review workflow.
...
**Gap discussion**: `system-design/02-prd/versions/round-0/01-gap-discussion.md`
**Author output**: `system-design/02-prd/versions/round-0/02-author-output.md`
**Updated draft**: `system-design/02-prd/versions/round-0/03-updated-prd.md`
...
├── prd.md                         # Promoted from create (then overwritten by Review promoter)
└── versions/
    ...
    └── round-0/
        ...
        └── 03-updated-prd.md                # Author output (if gaps exist)
```
...
- You DO NOT write draft PRD content, gap discussion content, or author output — agents do that
...
2. **If empty or no PENDING items**: Skip to Step 3b
...
1. **Check if brief document exists** at `system-design/02-prd/brief.md`
...
**On resume**: If Step 4 already marked complete, verify `00-draft-prd.md` exists and skip to Step 5. Do NOT re-run the Generator (it appends to downstream deferred items files and would create duplicates).
...
1. **Read the draft** at `system-design/02-prd/versions/round-0/00-draft-prd.md`
...
4. **Spawn Gap Formatter agent** using Task tool:
   ...
   Input:
   - Draft: system-design/02-prd/versions/round-0/00-draft-prd.md

   Output: system-design/02-prd/versions/round-0/01-gap-discussion.md
   ```

5. **Wait for Gap Formatter to complete**

6. **Verify output exists** at `system-design/02-prd/versions/round-0/01-gap-discussion.md`
...
8. **Spawn Gap Analyst agents** using Task tool (batch by section, same grouping as Discussion Facilitator):
   ...
   Gap discussion file: system-design/02-prd/versions/round-0/01-gap-discussion.md
...
9. **Notify user** gap analysis is ready for review:
   ...
   Discussion file: system-design/02-prd/versions/round-0/01-gap-discussion.md
...
11. **Discussion loop**:
    ...
    b. **Group gaps into batches** (2-3 batches recommended):
       - Group by PRD section number (gaps contain `**Section**: §N`)
       - Batch 1: Sections 1-4 (Goal, Success Criteria, Capabilities, Scope)
       - Batch 2: Sections 5-8 (Data Model, Key Decisions, User Workflows, Integration)
       - Batch 3: Sections 9-11 (Compliance, Risks, Definition of Done)
    ...
    c. **Invoke Discussion Facilitator agents** (one per batch, in parallel):
       ...
       Issues file: system-design/02-prd/versions/round-0/01-gap-discussion.md
...
14. **Spawn Author agent** using Task tool:
    ...
    Output:
    - Change log: system-design/02-prd/versions/round-0/02-author-output.md
    - Updated PRD: system-design/02-prd/versions/round-0/03-updated-prd.md
    ```

15. **Wait for Author to complete**

16. **Verify outputs exist**:
    - `system-design/02-prd/versions/round-0/02-author-output.md`
    - `system-design/02-prd/versions/round-0/03-updated-prd.md`
...
18. **Determine final draft path**:
    - If Steps 5-7 ran (gaps existed): Use `system-design/02-prd/versions/round-0/03-updated-prd.md`
    - If Steps 5-7 were skipped (no gaps): Use `system-design/02-prd/versions/round-0/00-draft-prd.md`

19. **Copy final draft to `prd.md`** using Bash cp:
    ```
    cp [final draft path] system-design/02-prd/prd.md
    ```

20. **Verify promotion** — Confirm `system-design/02-prd/prd.md` exists
...
23. **Present summary**:
    ...
    Draft: system-design/02-prd/versions/round-0/00-draft-prd.md
    [If gaps resolved:]
    Updated: system-design/02-prd/versions/round-0/03-updated-prd.md
...
**IMPORTANT**: The Review workflow reads from `system-design/02-prd/prd.md` for Round 1. This file is created by the promotion step (Step 8). It MUST exist before starting the Review workflow.
