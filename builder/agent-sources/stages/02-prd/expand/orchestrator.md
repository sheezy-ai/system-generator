# PRD Expand Orchestrator

---

## Purpose

Expand the PRD by adding new capability areas or scope changes to an existing, promoted document. Unlike Create (which generates from scratch) or Review (which critiques and refines), Expand adds new content driven by a specific trigger — a downstream discovery, a scope decision, or a human-identified gap.

The expand workflow produces an updated PRD that reads as if the expanded capability was always in scope. It does not promote — the next step is always a Review round.
...
**State file**: `system-design/02-prd/versions/workflow-state.md`
**Working files**: `system-design/02-prd/versions/round-[N]-expand/`
...
Note: Change verification for expand uses the review workflow's change-verifier agent (`{{AGENTS_PATH}}/02-prd/review/workflow/change-verifier.md`) since the verification logic is the same.
...
│   ├── 05-updated-prd.md              # Updated PRD with expansion applied
...
2. **Determine trigger input**:
   - If a pending issues file exists with unresolved issues: use `system-design/02-prd/versions/pending-issues.md`
...
3. **Run Scope Analyst**:
   ...
   Trigger: system-design/02-prd/versions/round-[N]-expand/00-trigger.md
   PRD: system-design/02-prd/versions/round-[N]-expand/00-prd.md
   Blueprint: system-design/01-blueprint/blueprint.md
   Stage guide: {{GUIDES_PATH}}/02-prd-guide.md
   Output: system-design/02-prd/versions/round-[N]-expand/01-expansion-brief.md
   ```
...
10. **Spawn Expansion Explorers** (one per capability area, in parallel):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/02-prd/expand/expansion-explorer.md

    Expansion Brief: system-design/02-prd/versions/round-[N]-expand/01-expansion-brief.md
    Capability area: [CAP-ID]
    PRD: system-design/02-prd/versions/round-[N]-expand/00-prd.md
    Blueprint: system-design/01-blueprint/blueprint.md
    Stage guide: {{GUIDES_PATH}}/02-prd-guide.md
    Output: system-design/02-prd/versions/round-[N]-expand/02-explorer-{cap-name}.md
    ```
...
12. **If multiple capability areas** (2+), run Exploration Consolidator:
    ...
    PRD: system-design/02-prd/versions/round-[N]-expand/00-prd.md
    Output: system-design/02-prd/versions/round-[N]-expand/03-consolidated-proposals.md
    ```
...
17. **Run Proposal Filter** to check level-appropriateness and format for human review:
    ```
    Follow the instructions in: {{AGENTS_PATH}}/02-prd/expand/proposal-filter.md

    Stage guide: {{GUIDES_PATH}}/02-prd-guide.md
    Input: system-design/02-prd/versions/round-[N]-expand/03-consolidated-proposals.md
    Output: system-design/02-prd/versions/round-[N]-expand/03-expansion-review.md
    ```
...
21. **Notify user** proposals are ready for review:
    - Point them to `system-design/02-prd/versions/round-[N]-expand/03-expansion-review.md`
...
23. **Discussion loop**:
    ...
    b. **Invoke Discussion Facilitator agents** (batched by capability area):
       ...
       Issues file: system-design/02-prd/versions/round-[N]-expand/03-expansion-review.md
       Issues: [ID1, ID2, ...]
       ```
...
27. **Run Integration Author**:
    ```
    Follow the instructions in: {{AGENTS_PATH}}/02-prd/expand/integration-author.md

    PRD: system-design/02-prd/versions/round-[N]-expand/00-prd.md
    Approved proposals: system-design/02-prd/versions/round-[N]-expand/03-expansion-review.md
    Stage guide: {{GUIDES_PATH}}/02-prd-guide.md
    Change log output: system-design/02-prd/versions/round-[N]-expand/04-integration-output.md
    Updated PRD output: system-design/02-prd/versions/round-[N]-expand/05-updated-prd.md
    ```
...
31. **Spawn all four verification agents in parallel**:
    ...
    **Change Verifier** (Step 6):
    ```
    Follow the instructions in: {{AGENTS_PATH}}/02-prd/review/workflow/change-verifier.md

    Issues summary with resolutions: system-design/02-prd/versions/round-[N]-expand/03-expansion-review.md
    Author output: system-design/02-prd/versions/round-[N]-expand/04-integration-output.md
    Updated PRD: system-design/02-prd/versions/round-[N]-expand/05-updated-prd.md
    Output: system-design/02-prd/versions/round-[N]-expand/06-change-verification.md
    ```
    ...
    **Alignment Verifier** (Step 7):
    ...
    Updated PRD: system-design/02-prd/versions/round-[N]-expand/05-updated-prd.md
    Blueprint: system-design/01-blueprint/blueprint.md
    Output: system-design/02-prd/versions/round-[N]-expand/07-alignment-report.md
    ```
    ...
    **Internal Coherence Checker** (Step 8):
    ...
    Document: system-design/02-prd/versions/round-[N]-expand/05-updated-prd.md
    Stage guide: {{GUIDES_PATH}}/02-prd-guide.md
    Output: system-design/02-prd/versions/round-[N]-expand/08-coherence-report.md
    ```
    ...
    **Enumeration Verifier** (Step 9):
    ...
    Document: system-design/02-prd/versions/round-[N]-expand/05-updated-prd.md
    Stage guide: {{GUIDES_PATH}}/02-prd-guide.md
    Output: system-design/02-prd/versions/round-[N]-expand/09-enumeration-report.md
    ```
