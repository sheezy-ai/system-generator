...
Because the promoter is the **sole producer of `prd.md`** and Promote is the only workflow that runs it, this sits on the **only road** to a promoted PRD. Promote is a plain split-and-record — the PRD has no downstream contract registry, so there is no freeze gate, materialization, or fidelity check (those are 04-only).
...
└── promoter.md                         # Splits the reviewed PRD into prd/decisions/future
```
...
├── prd.md                         # Clean current-scope (created by the promoter here)
├── decisions.md                   # Product decision rationale (created by the promoter here)
├── future.md                      # Deferred features (created by the promoter here)
└── versions/
    ├── workflow-state.md
...
**Review-mandatory guard (structural — this makes "Review before a usable `prd.md`" structural, not convention):**
...
10. **Copy the promoted set into the round record**: copy `prd.md`, `decisions.md`, `future.md` into `round-[N]-promote/` (the working record). The parent copies are the published authority; these are the frozen snapshot for this round.
...
17. **Report** to the user: the split is complete; the three published documents (`prd.md`, `decisions.md`, `future.md`) are current, and `round-[N]-promote/` holds the record.
