...
├── round-{N}-create/              # Creation workflow outputs
│   ├── explore/
│   └── [generate files]
...
2. **Create round directory and copy input**:
   - Create `system-design/02-prd/versions/round-[N]-review/`
...
3. **Spawn expert agents in parallel** using Task tool
   - Pass to each agent:
     - PRD file path: `system-design/02-prd/versions/round-[N]-review/00-prd.md`
   ...
   - Agents write directly to `system-design/02-prd/versions/round-[N]-review/01-[expert-name].md`
...
8. **Run Consolidator** (MUST spawn as subagent using Task tool - do NOT perform inline)
   ...
   - Agent writes to `system-design/02-prd/versions/round-[N]-review/02-consolidated-issues.md`
...
12. **Run Scope Filter agent** (MUST spawn as subagent using Task tool - do NOT perform inline)
    ...
    Input: system-design/02-prd/versions/round-[N]-review/02-consolidated-issues.md
    Output: system-design/02-prd/versions/round-[N]-review/03-issues-discussion.md
    ```
...
16. **Run Issue Analyst agents** (MUST spawn as subagents using Task tool - do NOT perform inline)
    ...
    Context documents:
    - PRD: system-design/02-prd/versions/round-[N]-review/00-prd.md
    - Blueprint: system-design/01-blueprint/blueprint.md

    Issues file: system-design/02-prd/versions/round-[N]-review/03-issues-discussion.md
    Issues: [ID1, ID2, ID3, ...]
    ```
...
21. **Notify user** issues are ready for discussion
    - Point them to `system-design/02-prd/versions/round-[N]-review/03-issues-discussion.md`
...
27. **Run Author agent**
    ...
    - Agent writes to `system-design/02-prd/versions/round-[N]-review/04-author-output.md` (changelog) and `system-design/02-prd/versions/round-[N]-review/05-updated-prd.md`
...
31. **Spawn all four verification agents in parallel**:
    ...
    **Change Verifier** (Step 6):
    ...
    - Agent writes to `system-design/02-prd/versions/round-[N]-review/06-change-verification-report.md`
    ...
    **Alignment Verifier** (Step 7) (`{{AGENTS_PATH}}/universal-agents/alignment-verifier.md`):
    ...
    - Agent writes to `system-design/02-prd/versions/round-[N]-review/07-alignment-report.md`
    ...
    **Internal Coherence Checker** (Step 8) (`{{AGENTS_PATH}}/universal-agents/internal-coherence-checker.md`):
    ...
    - Agent verifies cross-section consistency within the PRD
    - Agent writes to `system-design/02-prd/versions/round-[N]-review/08-coherence-report.md`
    ```
    Follow the instructions in: {{AGENTS_PATH}}/universal-agents/internal-coherence-checker.md

    Document: system-design/02-prd/versions/round-[N]-review/05-updated-prd.md
    Stage guide: guides/02-prd-guide.md
    Output: system-design/02-prd/versions/round-[N]-review/08-coherence-report.md
    ```
    ...
    **Enumeration Verifier** (Step 9) (`{{AGENTS_PATH}}/universal-agents/enumeration-verifier.md`):
    ...
    - Agent writes to `system-design/02-prd/versions/round-[N]-review/09-enumeration-report.md`
    ```
    Follow the instructions in: {{AGENTS_PATH}}/universal-agents/enumeration-verifier.md

    Document: system-design/02-prd/versions/round-[N]-review/05-updated-prd.md
    Stage guide: guides/02-prd-guide.md
    Output: system-design/02-prd/versions/round-[N]-review/09-enumeration-report.md
    ```
...
46. **Handle pending issue sync based on decision**:
    ...
    b. **If "Sync now" or "Select individually"**:
       - Run Pending Issue Resolver agent:
         ...
         Alignment report: system-design/02-prd/versions/round-[N]-review/07-alignment-report.md
         ...
         Output: system-design/02-prd/versions/round-[N]-review/10-pending-issue-sync.md
         ```

46b. **Close PRD-stage pending-issues for entries resolved this round**:

    This step closes entries in the **current stage's own** pending-issues file (`system-design/02-prd/versions/pending-issues.md`). It is distinct from step 46's Pending Issue Resolver, which writes to **upstream** pending-issues files (Blueprint).

    Procedure:
    - Read `system-design/02-prd/versions/pending-issues.md`
    - Read this round's `02-consolidated-issues.md` to identify which Unresolved entries were merged into this round's issue stream by the Consolidator at Step 2 (these will be the entries with their original PI-NNN / SPEC-NNN IDs mapped to round-local issue IDs)
    ...
    - For each merged-and-resolved entry:
      ...
      - Add resolution metadata to the entry:
        - `**Status:** RESOLVED`
        - `**Resolved:** [date]`
        - `**Resolution Round:** PRD Round [N] Review (issue tracked as [round-local ID])`
        - `**Resolution:** [one-paragraph note explaining how the round closed the entry — APPLIED edits reference the PRD section; close-with-no-change references the human-approved recommendation; close-as-resolved-by-prior-round references the earlier round's issue ID that closed it]`
    - **Also record mainline dismissals (re-raise ledger)**: for each **expert-raised issue this round** (a round-local PRD-NNN that was **not** already a pending-issue) whose resolution outcome in `03-issues-discussion.md` is **close-with-no-change** or **close-as-stale** (dismissed / won't-fix / working-as-intended — a resolution that leaves **no trace in the PRD text**), **add a new entry** to the `## Resolved Issues` section with:
      - `**Status:** RESOLVED (dismissed — no document change)`
      - `**Resolved:** [date]` · `**Resolution Round:** PRD Round [N] Review`
      - `**Concern key:** [PRD section anchor] — [one-line concern summary]` — this is the **stable key the Consolidator matches against to suppress re-raises**; use the document section + concern gist, because round-local PRD-NNN IDs do not persist across rounds
      - `**Resolution:** [why it was dismissed — the human-approved rationale]`
      APPLIED-inline resolutions do **not** need a ledger entry (the PRD text changed, so a future round sees the fix directly). It is specifically the **no-document-change dismissals** that must be recorded — otherwise the unchanged PRD text invites the identical concern to be re-raised next round, which is the review loop's non-convergence trap.
    ...
    Rationale: this step ensures the PRD-stage pending-issues file reflects the truth of what each round closed. Without it, downstream stages reading this file see stale Unresolved entries that have actually been addressed. The orchestrator separates this from step 46's Pending Issue Resolver because the Resolver's semantic is "log findings to the **upstream** register (Blueprint)", whereas this step's semantic is "close entries in our **own** pending-issues register".
