...
**Update (see DEC-081):** The "at exit" mechanism described here has since been decoupled — promotion is now a separate **Promote** workflow (stages 02–04), not the final step of Review. The split logic and the three-file output are unchanged; only *where* the promoter runs moved.
...
**Rationale:** 04's Promote must be a distinct, re-runnable act — it is the contract freeze that materializes stage 05's registry, runs on every promote including backward-edge re-promotes, and cannot live inside a Review round. Keeping 02/03 promoting *inside* Review while 04 promoted as its own stage left the pipeline inconsistent (which workflow promotes, and when, differed by stage). Decoupling 02/03 to match 04 makes create/review/promote a uniform three-workflow shape across 02–04, makes "Review before a usable published spec" a structural guarantee (the guard) rather than a convention, and gives every promotion an auditable `round-N-promote` record. The lighter alternative (keep promote in-review, emit a record from there) was rejected because it would preserve the 02/03-vs-04 inconsistency. See DEC-072 for the split itself.

---

### DEC-082: Promote as the Single Contract Freeze

**Decision:** Materialization of the cross-cutting contract registry (`05-components/specs/cross-cutting.md`, projected from Architecture §7/§8 + PRD §5) and its fidelity check run **inside the Architecture Promote stage**, not at stage-05 initialization. Promote is a one-act freeze — gate → split → **materialize** → **fidelity** → publish → record — producing the complete + freezable + materialized + fidelity-verified contract authority in a single act; 05-init only *consumes* the frozen registry (a registry-present precondition). Supporting rules:

- **Status-preserving re-freeze (MERGE):** a re-promote (backward edge `expand → review → promote`) merges rather than clobbers — two-track change detection, reset changed contracts to `DEFINED`, never keep `VERIFIED` across a changed obligation, no HALT. The registry is co-owned: Promote births the immutable **obligations**; stage 05 mutates only the **status** (`MATERIALIZED → DEFINED → VERIFIED`).
- **Round-first write-order:** the registry original is written to `round-N-promote/` and published to `05-specs` only after the fidelity check returns CLEAN (a single publish point — 05 never consumes an unverified registry). The two freeze agents (`contract-materializer`, `materialization-fidelity-checker`) live under `04-architecture/promote/`.
- **Legacy deletion:** the pre-materialization status family (`DEFERRED` / `POPULATED` / `PENDING_REGISTRATION` / fresh-population) was removed as dead-by-construction once materialization is mandatory at Promote.
- **Freeze-identity token + stale-registry guard:** Promote stamps a `Frozen-At` (round-N-promote) token into both `architecture.md`'s header and the registry Status block, tying the registry to the exact architecture freeze that produced it; every promote re-publishes and re-stamps unconditionally. 05-init errors if `registry.Frozen-At ≠ architecture.Frozen-At` — catching a registry stale versus the current architecture (e.g. after a promote HALT) mechanically, rather than by human sequencing.
- **Write-direction re-verify flag:** on a MERGE re-freeze, Promote flags every producer of a changed write-direction contract with a durable `CROSS-BOUNDARY-PEER` / HIGH pending-issue, consumed at that producer's next review (Step 0 + the contract-verifier).
...
### DEC-083: Retire the Legacy Conformance Workflow; Body-Driven Absent-From-Freeze Detection
...
### DEC-084: Contract-First — Obligations Frozen Upstream, Realizations Constrained Downstream

**Decision:** Inter-component contracts are frozen at design time, upstream, at the Architecture ↔ Component-Specs seam — **not** discovered inline while a component body is written. The governing principle:

> Freeze contract **obligations** upstream (Architecture §7/§8); keep constrained **realizations** in the components; **escalate** unmade upstream decisions back to Architecture rather than resolving them in a body.

Every cross-component contract falls into one of three buckets:

1. **Frozen obligation + constrained realization** — the obligation/invariant is pinned upstream; only a fenced realization (encodings, table names, literal values) is component-owned, constrained so it cannot break a consumer.
2. **Unmade-upstream-decision (D)** — a real decision not yet made, belonging at Architecture/Foundations; **escalated** back (the Expand / `pending-issues` backward edge), never discovered in a body.
3. **Genuine emergence (C)** — a contract determinable *only* by working a running body.

**Rationale:** A component spec and a contract are the *same kind of artifact* — both are design-level *decisions*. "Emergent from the body" only has force when the body is a *different* kind of artifact (running code) revealing *empirical* facts (performance, data distributions, framework limits); a spec reveals none of those, so every part of a contract can be decided in the design layer ahead of the bodies that consume it. A pressure-test of cross-component edges, deliberately biased toward where emergence would most likely hide (explicitly-delegated decisions, known-open questions, decoupling contracts), found bucket (C) **empty** — every case was either a frozen obligation or an unmade-upstream-decision. That empty (C) bucket is what licenses an aggressive contract-first posture: the "TBD-in-component" frontier is near-empty, so the real work is (a) freezing obligations *completely* and (b) handling (D) by escalation, not body-discovery. "Obligations up, realizations down" is also the altitude rule Architecture review already enforces — pinning realizations upstream would turn Architecture into a component spec and forfeit the stability that makes it authoritative.

This is the **foundational principle** beneath the promote-stage contract machinery: the completeness/freezability gate (DEC-081) checks obligations are complete and freezable; the Promote freeze (DEC-082) materializes the frozen obligations into the component-facing registry; the absent-from-freeze detector (DEC-083) catches a body that realizes or consumes an obligation missing from the freeze and escalates it as a (D). See also the spec/decisions/future split (DEC-072).

---

### DEC-085: Document-Conservation Gate at the Promote Split

**Decision:** Every Promote workflow that **splits** a reviewed document into three published files (02 PRD, 03 Foundations, 04 Architecture, 05 Components — 01 copies, so it is excluded) runs a **document-conservation gate** on the split, via one **universal** agent (`universal-agents/document-conservation-checker.md`) parameterized by a per-stage **verbatim-critical sections** list. Supporting rules:

- **Round-first split.** The promoter writes the three documents to the `round-[N]-promote/` folder as originals; the orchestrator **publishes** them to the live paths only **after the conservation gate returns CLEAN** — mirroring the DEC-082 registry write-order. This removes any window where a corrupted split is live and makes the gate a true pre-publish gate.
- **What gates vs. what is advisory.** The checker **GATES (HALTs the publish on MISMATCH)** on the mechanical, high-severity slice: (1) each **verbatim-critical section** is byte-identical (modulo HTML-comment removal) between the pre-split reviewed source and the clean spec, and (2) cross-references between the three docs resolve. It is **advisory (never HALTs)** on the judgment slice — transformed-prose / decisions / future survival and placement — with a `LOSSY`-style uncertain→advisory tier. Gating only the verbatim slice is deliberate: gating transformed content would reintroduce the paraphrase-undecidability the pipeline elsewhere refuses to gate on (DEC-058; the contract-materializer's "do not build semantic diffing").
- **Per-stage verbatim-critical sections:** 04 = §6 Component List, §8 Data Contracts, §7 interface-contract text; 05 = §3 Interfaces, §4 Data Model, §7 Integration; 02 = §5 Conceptual Data Model, §8 Integration Points, §9 Compliance and Constraints; 03 = §1–10 (all normative conventions; everything except §11 Open Questions).
- **Foundations promoter narrowing.** Because Foundations is normative-conventions end-to-end, its promoter is **narrowed** so its only §1–10 edits are HTML-comment removal; the sole moved content is the standalone `## Design Decisions` block + §11. Inline convention rationale **stays inline** — no rule forces it out (that would contradict the Foundations guide). This makes §1–10 genuinely verbatim and gateable at near-zero content loss.
- **Guard-before-mutate.** The review-mandatory guard runs at **On Start, before any state mutation** (05 parity), so a Promote mis-pointed at an unreviewed draft errors without stranding the stage in `Promote`/`IN_PROGRESS`.
