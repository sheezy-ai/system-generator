...
**Rationale:** Before this, a contract could be frozen at Architecture (§8) but only checked for completeness at stage 05, so an uncontracted cross-component read (specimen `search_vector` / DD-010) could fall through the 04/05 seam. Moving materialization + fidelity into Promote puts the whole freeze on the *only road* to a promoted `architecture.md`, closing the seam; the freeze-identity token converts "did 05 consume the registry matching this architecture?" from a sequencing assumption into a guard; round-first write-order guarantees 05 never sees an unverified registry. Builds on the contract completeness/freezability gate (see DEC-081) and the spec/decisions/future split (DEC-072). *(Slices 4–6.)*
