...
The **step-0 cross-cutting interfaces are real** — Architecture §6/§7 order them before component specs — and their **authority is Architecture §7**. A component **may** forward-commit to a step-0 cross-cutting interface: it records the commitment as *adopt-by-reference to §7* (e.g. "adopts a step-0 cross-cutting interface convention by reference"), which **resolves by reference to §7**. `cross-cutting.md` is **materialized up-front from Architecture §7/§8** by the Architecture **Promote** stage (the contract freeze) — a projection of the already-frozen contracts, authoritative only *by reference* to §7/§8, and consumed by Component Specs (05-init reads the frozen registry; it does not produce it). It is **not** an independent authority; conformance of the realized bodies against it is verified in coherence Phase 4.
