...
It replaces the retired forward-commitment **ratification** model — specifically the part that assumed a separate step-0 cross-cutting-spec *author* would later "ratify" downstream commitments. That ratifying party was never built and is the fiction being retired. The **step-0 cross-cutting interfaces themselves are real** — Architecture §6/§7 order them before component specs — and their **authority is Architecture §7**. So a component **may** forward-commit to a step-0 cross-cutting interface: it records the commitment as *adopt-by-reference to §7* (e.g. "adopts a step-0 cross-cutting interface convention by reference"), which **resolves by reference to §7**, not by a phantom ratifier. What does not exist is the separate ratifying author. `cross-cutting.md` is **materialized up-front from Architecture §7/§8** by the Architecture **Promote** stage (the contract freeze) — a projection of the already-frozen contracts, authoritative only *by reference* to §7/§8, consumed by Component Specs (05-init reads the frozen registry; as of Slice 4 it no longer produces it) as a resolved contract layer. It is still **not** an independent authority and no party ratifies it; conformance of the realized bodies against it is now verified in coherence Phase 4 (the post-promotion cross-cutting population workflow that formerly reconciled bodies against it has been retired).
...
- **Absent-from-freeze detector (create round-0 + every review round)** — scans the body's produced **and consumed** cross-component interfaces and escalates any that is **absent from the frozen registry** as a P2 (`CROSS-BOUNDARY-UPSTREAM`) to Architecture for the next re-freeze; it never registers a contract locally. This is what catches a cross-component read the freeze missed (the consumer-side scan is the load-bearing case). See DEC-083.
