...
## 4. What already exists (the seam is ~80% built)
...
- **Contract registry + conformance** — `05-components/cross-cutting/` (`contract-extractor`, `contract-reconciler`) and `review/contract-verifier.md` exist to extract contracts into `cross-cutting.md` and reconcile consumer↔producer (currently `DEFERRED`).
