...
- **Act — human-triggered and human-judged (deferred as *automation*, not as detection).** Re-decomposition is a genuine design decision no agent should make, so the action is a **HALT-to-human**: the detector advises, the human decides the new decomposition and triggers it. The *automated* incremental re-decomposition path is the deferred piece — 05 uniquely has **no Expand workflow** (01–04 do; 05 does not) and re-initialization is **destructive** (`05-components/initialize/orchestrator.md:345`), so building a non-destructive 05 decomposition-reconciliation is a separate, larger follow-up. What the design delivers is: **detection closed (loud), the action human-owned** — which is the correct split, since the action is exactly the design judgement humans should keep.
  - **Update (2026-07-21) — the *retire* slice of the ORPHANED action shipped.** The ORPHANED leg now has a defined, non-destructive, human-triggered action: the `05-components/retire/orchestrator.md` orchestrator (guarded blocking sweep + archive-not-delete + status-less index audit + directs re-coherence). See `docs/05-orphaned-retirement-design.md`. This is a *human-triggered* action (not an auto-mutation loop) — the re-decompose *decision* stays human, consistent with the split above. Still deferred: **re-split / merge / re-parent** (the R8 remainder — genuine re-decomposition, human-driven by hand). MISSING was already actionable via per-component Create.
...
- **R8 — Tier-2 *action* deferred (decision D, accepted). *Partially closed 2026-07-21: the retire slice shipped.*** Detection ships now (loud missing/orphaned); the **automated** incremental re-decomposition does **not** — 05 has no Expand and re-init is destructive (§3.7), so the action is HALT-to-human (the re-decompose decision is human judgement anyway). **Update:** the **ORPHANED retire slice** now has a defined non-destructive human-triggered action — `05-components/retire/orchestrator.md` (guarded sweep → archive-not-delete → status-less index audit → directs re-coherence; see `docs/05-orphaned-retirement-design.md`). MISSING was already actionable via per-component Create. **Still deferred (the R8 remainder): re-split / merge / re-parent** — genuine re-decomposition, human-driven by hand. Note the retire slice is deliberately *human-triggered*, not an auto-mutation loop — so §9's "no auto-mutation loop / no new reconciliation workflow" boundary still holds (retire is a human-invoked stage op, like coherence).
...
- The **automated** 05 re-decomposition action for a Tier-2 hit (a non-destructive decomposition-reconciliation; 05 has no Expand) — R8. *(Partially closed 2026-07-21: the ORPHANED **retire** slice shipped as `05-components/retire/orchestrator.md` — human-triggered, non-destructive. Remaining: re-split / merge / re-parent, still human-driven by hand.)*
